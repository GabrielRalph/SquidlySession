...
async function runProcess(name = "default") {
	if (name in Process) {
		const input = { video: Video, canvas: Canvas, context: Ctx };
		...
		for (const listener of processListeners[name]) {
			try {
				listener(input);
			} catch (e) {
				console.log(e);
...
function isRunning() {
	for (const key in ProcessRunning) {
...
async function runProcessingLoop() {
	...
	while (!stopCapture) {
		...
		if (currentTime != lastTime) {
			// captureFrame();
			const proms = Object.keys(ProcessRunning)
...
export async function updateSelectedDevice(type, deviceId) {
	const [audioinput, videoinput, audiooutput] = await Promise.all([
		getSelectedDevice("audioinput"),
		getSelectedDevice("videoinput"),
		getSelectedDevice("audiooutput"),
	]);
...
export async function startWebcam(params = camParams1) {
	...
	try {
		...
		const stream2 = await navigator.mediaDevices.getUserMedia(camParams2);
...
export function stopWebcam() {
	try {
		for (const track of VideoOnlyStream.getTracks()) {
			track.stop();
		}
	} catch (e) {}
	stopProcessingAll();
...
export function isProcessing(name = "default") {
	return capturing && ProcessRunning[name];
}
